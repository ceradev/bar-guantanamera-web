    ...
    return (
      <div className="space-y-4">
        <div className="space-y-2">
          <div className={`${isMobile ? "text-sm" : "text-base"} text-gray-900`}>Nombre de la persona que recoge el pedido</div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <div className={`${isMobile ? "text-sm" : "text-base"} text-gray-900`}>Teléfono</div>
            <div className={`${isMobile ? "text-xs" : "text-sm"} text-red-500`}>Solo se requiere si el total supera 30€</div>
          </div>
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              type="tel"
              disabled={total <= 30}
              placeholder="Introduce el teléfono móvil"
              className={`w-full rounded-lg border border-gray-300 bg-white ${isMobile ? "px-3 py-2 text-sm" : "px-4 py-3 text-base"} focus:outline-none focus:ring-2 focus:ring-red-600`}
            />
          </div>
          {total > 30 && !phone.trim() && (
